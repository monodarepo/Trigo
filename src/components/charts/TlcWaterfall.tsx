...
function montarLinhas(componentes: readonly ComponenteTLC[], totalRs: number): LinhaWaterfall[] {
  let acumulado = 0
  ...
  return (
    <div className="max-w-64 rounded-card border border-edge bg-card-2 px-3 py-2 text-xs shadow-card">
      ...
      <p className="tnums mt-1 text-gold-light">
        {linha.ehTotal ? 'Total' : 'Parcela'}: {formatBRL(linha.valor, { casas: 2 })}/t
      </p>
      {!linha.ehTotal && (
        <p className="tnums mt-0.5 text-ink-muted">Acumulado: {formatBRL(linha.acumulado, { casas: 2 })}/t</p>
      )}
...
export function TlcWaterfall({ componentes, totalRs, ariaLabel }: TlcWaterfallProps) {
  const linhas = montarLinhas(componentes, totalRs)
    ...
    <div role="img" aria-label={ariaLabel}>
      <ResponsiveContainer width="100%" height={altura}>
        <BarChart data={linhas} layout="vertical" margin={{ top: 4, right: 56, bottom: 0, left: 8 }} barSize={14}>
          <XAxis
            type="number"
            domain={[0, Math.ceil((totalRs * 1.04) / 100) * 100]}
      ...
      <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-ink-subtle">
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-4 rounded-sm bg-gold/85" aria-hidden="true" />
          Custo firme
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-4 rounded-sm bg-info/85" aria-hidden="true" />
          Imposto
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-4 rounded-sm bg-danger/85" aria-hidden="true" />
          Risco precificado
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-4 rounded-sm bg-gold-light" aria-hidden="true" />
          TLC total
        </span>
      </div>
    </div>
