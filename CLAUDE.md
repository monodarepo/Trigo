...
## Alertas: uma fonte, seis superfícies
`src/alerts/alertStore.ts` é o STORE ÚNICO (semeado de `src/data/alertas.ts`); `selectors.ts` deriva tudo dele, **memoizado contra a referência da lista** — seletor que devolve array novo a cada chamada põe `useSyncExternalStore` em laço infinito (React #185) — e ordena sempre por `porUrgencia` (severidade → impacto → recência). Quem lê: o **sino** (`AlertBell`, badge = `filaExigeDecisao().length`, ponto rosa pulsante se houver crítico), a **Central** (`AlertCenter.tsx`, painel lateral global, sino ou tecla **N**), a **faixa crítica** (`CriticalBanner.tsx`), o **banner/chip contextual** (`AlertBanner.tsx`, em cada tela), o **detalhe** (`AlertDetail.tsx`, drawer global) e a **aba** `/alertas` (mission control). Nenhuma mantém lista própria.

**Uma ordem só** (`porUrgencia`): exige-decisão → severidade → impacto → recência. Trabalho pendente precede contexto; urgência não se compra com dinheiro; o dinheiro desempata dentro do nível. Central, aba, banner e chip usam a MESMA função — com duas ordenações, o "primeiro da fila" seria outro em cada tela.

**Clustering** (`clusters.ts`): sinais do mesmo fato viram uma linha expansível, com contagem visível. A chave é `eventoId` quando existe (mesmo FATO em objetos diferentes — o atraso do MV Río Paraná acende o navio e a cobertura de Natal) e a `entidade` quando não (mesmo OBJETO, fatos diferentes — os três de Rolândia). O cluster ocupa a posição do seu alerta mais urgente; agrupar antes de ordenar faria um grupo de médios passar na frente de um crítico solitário. O total do grupo só aparece com dois ou mais números **na mesma base e no mesmo sinal** — somar mês com trimestre, ou risco com oportunidade, daria um número que não é nem um nem outro.

**Um relógio só** (`TempoRelativo.tsx`): o "há Xs" vem do tick global do `liveStore`, e **apenas as linhas que chegaram ao vivo** o assinam — o alerta semeado mede contra a âncora do cenário, que é constante, e assinar o tick nele custaria 20 re-renders por segundo para reescrever o mesmo texto. Onde a hora só é lida no clique (trilha do VRO), usa-se `getLiveState()` em vez do hook.

**O alerta declara onde aparece** — `telasRelacionadas` e `entidade` —, a tela não filtra o catálogo. Foi assim que o Hedge deixou de perder o alerta de câmbio que vira a decisão de moagem (o filtro antigo era por categoria) e a Compra deixou de filtrar por texto do título. `<AlertBanner rota="/x" />` vai **logo abaixo do `SectionTitle`**, em todas as telas, com limite de 2 (a Visão Executiva usa 1: o herói é a decisão do dia) e "+N na Central" para o resto. `<AlertChip entidadeId={id} />` é a variante de cabeçalho de painel e segue o objeto selecionado — trocar o moinho do cenário no Make/Buy/Sell troca o alerta junto.

**Ciclo de vida, não notificação passiva.** O drawer (`AlertDetail.tsx`) é onde o alerta vira trabalho: **reconhecer** (sai da fila, o risco fica na tela), **adiar** com prazo escolhido (`acoes.ts` → pregão/24h/3 dias/1 semana), **atribuir** a uma área (sugerida pela CATEGORIA — câmbio é da Tesouraria, moagem é da Indústria), **resolver** e **reabrir**. Cada ação emite toast e escreve em `registroVro.ts`, que o VRO mostra como **"Alertas tratados nesta sessão"**. Esse total é **valor ENDEREÇADO, não capturado** — resolver o alerta de R$ 560 mil põe uma decisão na mesa, não R$ 560 mil no YTD; a trilha separa as bases (mês/trimestre/evento), pinta de cinza o impacto do que ainda não foi resolvido e nunca soma nos KPIs de captura. A aba `/alertas` é a **única** superfície onde o resolvido continua existindo (chip "Resolvidos"), e é de lá que se reabre.

**A aba é o mission control**, e as outras superfícies são recortes dela: resumo com contagem por severidade + risco e oportunidade do mês separados; a fila na MESMA ordem da Central (`posicaoNaFila`, não uma ordenação própria — senão o "primeiro da fila" seria outro em cada tela) com o split exige-decisão × informativo; busca, filtro por categoria/severidade e agrupamento por prioridade/categoria/objeto com reordenação animada; **heat strip** de quando cada categoria acendeu (as 13h anteriores, clicável para filtrar — é o que mostra vigilância contínua em vez de uma pilha sem tempo); e **ação em massa** (resolver/reconhecer/atribuir), que escreve na trilha do VRO uma linha por alerta, como o drawer.

Regras que não devem ser desfeitas: a Central lista **a fila de decisão**, não os ativos — por isso os chips de severidade decompõem o número do topo (somam-no) e o total de ativos vive no rodapé; o selo "novo" é só para o que chegou **durante a sessão** (`recebidoEmS`), senão os 18 alertas do primeiro acesso viram papel de parede; a chegada ao vivo (`chegadaAoVivo.ts`) publica no store e emite toast na cor da severidade, suprimido apenas quando o alerta já apareceria na Central aberta; a faixa crítica **pula o crítico que o banner da tela já mostra** (senão a mesma frase aparece duas vezes na mesma dobra — e, de quebra, cada tela passa a anunciar dois críticos diferentes); banner e chip escondem `adiado`/`reconhecido`, que já tiveram uma decisão. No banner, o **título abre o detalhe** e o **CTA executa a ação** — trocar isso faz o rótulo prometer uma coisa e entregar outra. As ações de ciclo de vida (resolver / adiar 24h / reconhecer) vivem no detalhe: é o único lugar de onde um alerta sai de todas as superfícies de uma vez. Atalhos: **A** aprova, **G A** navega para a aba, **N** abre a Central — as três coisas são diferentes.
...
- Monoda: `monoda-logo-para-fundo-escuro.png` (raiz) → cópia de uso `public/brand/monoda-logo-branco.png` (494×148, conteúdo de borda a borda, lockup completo com símbolo + wordmark, aspecto ~3,34:1). Aparece ao lado da M. Dias Branco no login em **48px (cliente) × 36px (Monoda)** — alturas diferentes de propósito. Igualar a caixa entorta o par: o wordmark da M. Dias ocupa 25,4% da altura do arquivo (o resto é a espiga, de traço fino) e o "MONODA" ocupa 33,8% em caixa alta cheia, então na mesma caixa a Monoda sai 1,3× maior. Com 48/36, o wordmark fica em 12,2px nos dois e as larguras em 125px e 120px. Mexer numa altura sem refazer a conta desfaz o par.
